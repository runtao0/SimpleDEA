...
class Pagination {
  ...
  _notifyPage() {
    if (this.order === 0) {
      this.pageCount.innerHTML = "";
      return;
    }
    this.pageCount.innerHTML = `(${this.page + 1}/${this.totalPages}) ${this.interval} per page`;
  ...
  _totalPages() {
    const totalPages = Math.ceil(this.order.length / this.interval);
    return totalPages;
  }
}
